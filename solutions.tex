...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{\ref{Equations in the group}.} 
Set 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{\ref{Closed polyhedral surface}.} 
\textit{Closed polyhedral surface.}
Start with you favorite convex polyhedron $K$.
Assume that the interior of $K$ contains the origin $0\in\RR^3$.

Remove from $K$ the interior of $K'=\tfrac12\cdot K$.

Note that one can drill from each vertex of $K$ a cylindrical polyhedral tunnels to the corresponding vertex $K'$
...
it is a bent version of so called \emph{Schwarz boot} \cite[][see]{schwarz}.
Start with an isometric piecewise linear embedding 
of cylinder as shown on the diagram;
the top and bottom curves are triangles and their planes meet at angle the angle $\tfrac\pi n$ for a positive integer $n$.
...
measured in the tangent cone to $\Sigma$ at $v$.

The curvature of one vertex of $\Sigma$, say $\kappa$, is defined as $2\cdot\pi-\theta$, where $\theta$ is total angle around the vertex.
For tetrahedron, octahedron, cube, icosahedron and dodecahedron 
$\kappa$ is
$\pi$, $\tfrac23\cdot\pi$ , $\tfrac12\cdot\pi$, $\tfrac13\cdot\pi$ and $\tfrac15\cdot\pi$ correspondingly.

By Gauss--Bonnet formula $\alpha$ 
is a multiple of $\kappa$ modulo $\pi$.


For tetrahedron, octahedron, cube and icosahedron, 
it follows that there is a rotational symmetry of $K$ which fix $v$ and sends $\xi$ to $\zeta$.
(In the case of tetrahedron, this rotation is the identity map; that is $\xi=\zeta$.)
