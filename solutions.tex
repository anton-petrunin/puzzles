...
\begin{lpic}[t(-0mm),b(-0mm),r(0mm),l(0mm)]{pics/tangent-eq-sol(1)}
\lbl[tr]{20,5;$u$}
\lbl[rt]{18,15;$r$}
\lbl[rt]{26,6;$l$}
\lbl[bl]{25,10;$x_u$}
...
\begin{lpic}[t(-7mm),b(2mm),r(0mm),l(0mm)]{pics/birkhoff(1)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{\wrenches\ref{Warped product}.} 
...
Assume that a point $x\in W$ projects to a point $x\in\Sigma$.
Straightforward computations show that
\begin{align*}
\Sc_W(y)
&=\Sc_\Sigma(x)-2\cdot\frac{\Delta f(x)}{f(x)}=
\\
&=\Sc_M(x)-2\cdot\kappa_x^2-2\cdot\Ric(\nu_x)-2\cdot\frac{\Delta f(x)}{f(x)}.
\end{align*}
Consider linear operator 
\[L\:f(x)\mapsto [-\kappa_x^2-\Ric(\nu_x)]\cdot f(x)-\Delta f(x)\]
It is sufficient to find a function $f$ on $\Sigma$ such that
\[(Lf)(x)\ge 0\leqno({*})\]
for all $x\in \Sigma$.


Fix a smooth function $f\:\Sigma\to \RR$.
Let us extend the field $f(x)\cdot\nu_x$
from $\Sigma$ to a smooth field, say $v$ on whole $M$.
...
Then the value $(Lf)(x)$ is the derivative of
the function $t\mapsto H_t(x)$  at $0$.
The existance of $f$ with this property should be evident;
if not read further.
...
Note that this is an eigen function 
for the operator 
\[L\:f\mapsto [-\kappa_x^2-\Ric(\nu_x)]\cdot f+\Delta f,\]
denote by $\lambda$ its eigen value;
by $({*}{*})$,
$\lambda\ge 0$.

Prove that $f(x)>0$ at any $x$.
It follows that $Lf=\lambda f>0$.




\parit{Comments.}
It is is due to Mikhael Gromov and Blaine Lawson, see \cite{gromov-lawson}.
Earlier Shing-Tung  Yau and Richard Schoen swoed that the same assumptions 
impy existance of conformal factor on $\Sigma$ which makes it positively curved.
Both statements admit straightforward generalizations to the higher dimensions.
Both can be used 
to proof that 3-torus does not admit a metric with positive scalar curvature;
the original proof is given in \cite{schoen-yau}.
The same proof works in dimension up to 7;
in dimesnion 8 and higher it stop to work since the area minimizing hypersurfaces start to have singularities, see ???.
...
%\begin{center}
\begin{lpic}[t(-0mm),b(0mm),r(0mm),l(0mm)]{pics/milnors-discs()}
%\lbl[b]{24,41;x}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{\wrenches\ref{Complicated circle}.} 
\textit{Complicated circle.}
Acontinuous function $f\:[0,1]\to [0,1]$
will be called $\eps$-crooked 
if $f(0)=0$, $f(1)=1$ 
and for any segment $[a,b]\subset [0,1]$ 
one can choose $a\le x\le y\le b$ 
such that
\[|f(x)-f(a)|,|f(y)-f(b)|\le\eps.\]

A $\tfrac1n$-crooked maps $f_n\:[0,1]\to [0,1]$ can be constructed recurcevely. 
Guess the construction from the diagram.

%\begin{wrapfigure}{r}{50mm}
\begin{center}
\begin{lpic}[t(-0mm),b(4mm),r(0mm),l(0mm)]{pics/crooked(1)}
...
Construct recursevly a sequence of smooth simple closed curves $\gamma_n\:[0,1]\to\RR^2$.
...
and if one choose $\eps_n$ converging to zero fast enough
then the bondary of $D$ contains no simple curves.

\parit{Comments.} The proof repeats the construction of pseudoarc 
given by Bronis\l{}aw Knaster in \cite{knaster}.
Let us list few similar problems.
\begin{itemize}
...
\item Construct a Cantor set in $\RR^3$ with non simply connected complement. This example was constructed by Louis Antoine in \cite{antoine}.
\item Construct an open 3-dimensional manifold which is contractible, 
but not homeomorphic to $\RR^3$. This example is called \emph{Whitehead manifold},
see \cite{whitehead}.
\end{itemize}
For more advanced examples seach for Dogbone space and Casson handle, Dogbone space.
