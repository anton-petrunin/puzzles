...
it was used earlier 
to bound on the total curvature
...
%\begin{center}
\begin{lpic}[t(-0 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/kneser-log(.5)}
%\lbl[b]{24,41;x}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
Such example can be obtained by fattening Bing's house,
...
In fact there are Riemannian metrics on $\mathbb{S}^3$ 
...
%\begin{center}
\begin{lpic}[t(-0 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/circrooked(1)}
...
Let us list few problems which can be solved the same way.
\begin{itemize}
\item Construct three disjointed non-empty open sets in $\RR$ which have the same boundary.
\item Construct three open discs in $\RR^2$ which have the same boundary.
...
\item Construct a Cantor set in $\RR^3$ with non simply connected complement. 
...
\begin{lpic}[t(-5 mm),b(-3 mm),r(0 mm),l(0 mm)]{pics/serpinski-cemetery(1)}
\end{lpic}
\end{wrapfigure}
\parbf{\ref{Fat curve}.}
\textit{Fat curve.} 
...
see the 8th iteration on the diagram. 
...
\begin{lpic}[t(-0 mm),b(-0 mm),r(0 mm),l(0 mm)]{pics/tangent-eq-sol(1)}
\lbl[tr]{20,5;$u$}
\lbl[rt]{18,15;$r$}
\lbl[rt]{26,6;$l$}
\lbl[bl]{25,10;$x_u$}
...
that the minimal disc bounded by a simple closed curve with total curvature $\le 4\cdot\pi$ is necessary embedded.
...
was obtained by 
Stephan Stadler. %???REF
This result also implies the result of John Milnor that any embedded circle of total curvature at most $4\cdot\pi$ is unknot, see \cite{milnor}.
...
\begin{lpic}[t(-2 mm),b(2 mm),r(0 mm),l(0 mm)]{pics/birkhoff(1)}
...
%\begin{center}
\begin{lpic}[t(-2 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/ass(1)}
%\lbl[b]{24,41;x}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
\textit{Positive curvature and symmetry; prerequisite.}
...
Note that if  $G$ is a compact subgroup of isometries of a metric space $X$ then the quotient map $X\to X/G$ is a submetry,
assuming that the orbit space $X/G$ equipped with Hausdorff metric on the orbits.
...
\begin{itemize}
\item If $(M,g)$ is a Riemannian manifold with sectional curvature $\ge 1$ which admits a continuous isometric action of $\mathbb S^1$ then $(M,g)/\mathbb S^1$ is an Alexandrov space with curvature $\ge 1$.
\end{itemize}
...
The problem is due to Christopher Croke, see \cite{croke-energy}. 
...
\textit{Curvature vs. injectivity radius; prerequisite.}
The solution use a corollary from Liouville's theorem 
which states that geodesic flow on the tangent bundle to a Riemannina manifold preserves the volume form.
...
Given $r>r_0$, choose a point $q$ on the distance $2\cdot r$ from $p$.
 ...
 \begin{wrapfigure}[5]{o}{30 mm}
%\begin{center}
\begin{lpic}[t(-4 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/calabi-yau(1)}
\lbl[t]{9.5,8;$p$}
\lbl[t]{28,2;$q$}
\lbl[b]{10,20;$\partial B(p,r)$}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
\textit{Minimal foliation; prerequisite.}
...
\textit{Volume and convexity; prerequisite.}
The solution use a corollary from Liouville's theorem 
...
Note that 
there is a nonempty bounded open set $\Omega\subset \U M$
...
\begin{lpic}[t(-6 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/spiral}
\end{lpic}
\end{wrapfigure}
...
%\begin{center}
\begin{lpic}[t(-3 mm),b(-0 mm),r(0 mm),l(0 mm)]{pics/tree(1)}
%\lbl[b]{24,41;x}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
Take $X$ to be the set of non-negative integers with the metric $\rho$ defined as 
$\rho(m,n)=m+n$ for $m\ne n$.
...
The problem above was suggested by Linus Kramer.
...
\textit{Sub-Riemannian sphere; prerequisite.}
...
The Rademacher's theorem appears in \cite{rademacher}.%; see also ???. 
...
\begin{wrapfigure}[5]{o}{46 mm}
%\begin{center}
\begin{lpic}[t(-6 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/milnors-discs()}
%\lbl[b]{24,41;x}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
%\begin{center}
\begin{lpic}[t(-0 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/annuli()}
%\lbl[b]{24,41;x}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
\begin{wrapfigure}{o}{46 mm}
%\begin{center}
\begin{lpic}[t(-0 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/Phi(1)}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
see \cite{alexandrov-finite} and \cite{mccord}.
...
\parit{Comments.} The problem was mentioned by Frederic Le Rox, see \cite{rox}.
...
\begin{lpic}[t(-0 mm),b(-3 mm),r(0 mm),l(0 mm)]{pics/tent}
\end{lpic}
\end{wrapfigure}
...
Moreover, the surface which minimize the area among all surfaces with fixed  triangulation may be not saddle. 
...
%\begin{center}
\begin{lpic}[t(-4 mm),b(1 mm),r(0 mm),l(0 mm)]{pics/Convex-triangulation(1)}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
Look at the neighborhood of the boundary components and show that the annulus can and can not be isometrically immersed into the plane;
...
by Stephanie Alexander, Vitali Kapovitch and me, see \cite{akp}.
...
a solution follows from the proof that a square can not be cut into triangles of equal area given by Paul Monsky in \cite{monsky}.
...
\textit{Right-angled polyhedron; prerequisite.}
