...
Show that for any $x\in K$
any triangle $\triangle x_ix_jx$ 
is either right or acute.
In particular each $x_i$ is a vertex of $K$.

Denote by $h_i$ the homothety with center at $x_i$ and coefficient $\tfrac12$.
Set $K_i=h_i(K)$.

Show that for any $i$ and $j$ the polyhedrons $K_i$ and $K_j$ have no common interior points. 

Note that $K_i\subset K$ for any $i$;
it follows that 
\begin{align*}
\tfrac m{2^n}\cdot \vol K&=\sum_i\vol K_i\le
\\
&\le\vol K.
\end{align*}
Hence the result follows.
