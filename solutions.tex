...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{\wrenches\ref{Twisted anulus}.} 
\textit{Twisted anulus.}
Set 
\begin{align*}
...
\[\hat f\:z\mapsto f(z)+\eps\]
has no fixed points.

Prove that there is a curve $\hat \gamma$ such that 
\[\hat f\circ\hat\gamma(t)=\hat\gamma(t+1)\ \ 
\text{and}\ \ 
\hat \gamma(\pm T)\in H_\pm
\]
for some big $T>0$.

Repeat the same construction for the function $\check f}(z)=f^{-1(z)+\eps$ and obtain a curve $\check \gamma$.

Consider two maps $\hat\theta,\check\theta\:[-T,T]\to\CC$
 defined by
\begin{align*}
\hat\theta\:t&\mapsto f(\hat\gamma(t))-\hat\gamma(t).
\\
\check\theta\:t&\mapsto f(\check\gamma(t))-\check\gamma(t).
\end{align*}
Note that $\hat\theta(-T)=\check\theta(-T)=-i$ 
and $\hat\theta(T)=\check\theta(T)=i$.

Since $f$ has no fixed points, the maps $\theta_\pm$ have to be homotopic in $\CC\backslash\{0\}$ rel ends. 
Arrive to a contradisction by showing that this is not so (look at the picture). 

\parit{Comments.}
The problem was proposed by by Poincare in \cite{poincare}
and solved by Birkhoff in \cite{birkhoff} with the same solution as above.
