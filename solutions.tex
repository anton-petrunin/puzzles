...
Without loss of generality, we may assume that the cube contains the unit ball centered at the origin.

Let us extend the homeomorphism $h$ to whole $\RR^m$ by reflecting the cube in its facets.
We get a homeomorphism say $\tilde h\:\RR^m\to\RR^m$ such that $\tilde h(x)=h(x)$ for any point $x$ in the cube and 
\[\tilde h(\gamma\cdot x)=\gamma\cdot \tilde h(x)\]
for any isometry $\gamma$ in the group generated by the reflections in the facets of the cube.

Notice that $\tilde h$ has \emph{finite displacement};
that is, there is a constant $D$ 
such that 
\[|\tilde h(x)-x|\le D\]
for any $x\in\RR^m$.
(One can take $D$ to be diameter of the cube.)
...
Set $f(x)=\Phi\circ \tilde h \circ \Phi^{-1}(x)$ if $|x|<2$
and $f(x)=x$ otherwise.
...
An analogous construction was used by Sullivan in \cite{sullivan} 
to prove the same for bi-Lipschitz homeomorphisms.
