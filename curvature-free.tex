...
\begin{pr}
Consider the product of spheres $\mathbb{S}^2\times \mathbb{S}^2$ equipped with a Riemannian metric $g$ 
...
Show that the maximal distance from $M$ to the points of $W$ can be bounded below by a positive constant $\eps_m$ which depends only on the dimension $m$ (in fact, $\eps_m=\tfrac2{m+3}$ will~do).
\end{pr}
...
Assume $\Delta_1,\Delta_2\subset X$ be two embedded $n$-discs which have the same boundary.
...
the coefficients of $p$, up to a multiplicative constant, the integrals along $M$ of the quantities called \emph{Lipschitz--Killing curvatures} --- these are certain scalars which can be expressed in terms of curvature tensor at the given point.
