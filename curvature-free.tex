...
It follows that $\bm{f}\:Q\to Q$ is onto.
...
Given a smooth submanifold $L\subset M$;
...
First note that there is a self-dual harmonic 2-form on $(\mathbb{S}^2\times\mathbb{S}^2,g)$;
...
We can choose an orthonormal basis $e_1, e_2, e_3, e_4$ on $\T_p$ so that 
\[\omega_p=\lambda_p\cdot e_1\wedge e_2+\lambda'_p\cdot  e_1\wedge e_2\]
...
\begin{lpic}[t(-0 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/tripod}
\end{lpic}
\end{wrapfigure}
...
Given $q\in L$, denote by $v_q$ the direction of a minimizing geodesic from $q$ to $p$.
Note that $v_q\notin \mathrm{N}_qL$.
...
Many of professional geometers I know were surprised by this result, so I decided to include it here. 
...
\begin{itemize}
\item[$({*})$] Given a positive integer $n$ there is a tree $T_n$ embedded into the disc $\DD$ such that any null-homotopy of $\partial \DD$ passes through a curve which intersects $n$ different edges.
\end{itemize}
...
the least length is at least $\tfrac{\eps}{10}\cdot n=\tfrac{1}{100}\cdot n$.
The obtained metric is not Riemannian, but is easy to smooth keeping this property.
...
Assume $\Delta$ be a simplex and $\Delta_v$ is its facet opposite to the vertex $v$.
Assume $f_v\:\Delta_v\to M$ is a map and $x\in M$ such that $f_v(\Delta_v)\subset B(x,1)$.
Given $w\in \Delta_v$ denote by $\gamma_w$ the minimizing geodesic path from $x$ to  $f_v(w)$.
...
is called {}\emph{cone over $f_v$} with the vertex $x$. 
...
Assume $\tfrac{1}{2\cdot(m+1)}>h$.
Apply above construction for $\eps<\tfrac{1}{2\cdot(m+1)}-h$.
