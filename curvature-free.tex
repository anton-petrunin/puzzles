...
some symplectic geometry, in particular that geodesic flow preserves the volume on the tangent bungle. 
...
The solution requires Novikov theorem on the algorithmic undecidability of the problem of recognition of the sphere $\mathbb S^m$  for
$m\ge 5$. 
...
It remains to smooth $\partial(\Delta\times \Delta)$.
\qeds
...
In each tunnel, shrink the metric in the direction of the vertical direction  and expand it in the horizontal directions while keeping the volume the same.
...
where $\phi=\phi(x,y)$ is a function on $\DD$ which takes huge values around the center and identically 1 near the boundary of $\DD$.
...
Consider the normal exponential map $\exp_M\:\mathrm{N} M\to\RR^m$
