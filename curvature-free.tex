...
The restriction of $\exp|_{\T_p}$ is called \index{exponential map}\emph{exponential map at} $p$ and denoted as $\exp_p$.

Given a smooth immersion $L\to M$;
denote by $\mathrm{N} L$ the normal bundle over $L$.
The restriction $\exp|_{\mathrm{N} L}$ is called {}\emph{normal exponential map} of $L$ and denoted as $\exp_L$.
...
It is not at all easy to find an example of a manifold that satisfy the above condition for some $L$;
...
Notes that it maps $Q$ to itself
...
assume further that any geodesic that comes into $K$ goes out from $K$ the same way as if the metric would be Euclidean everywhere. 
Show that $g$ is flat.
\end{pr}

The same proof gives a rough version of this inequality holds for any metric on the cube.
Here is one of its applications suggested by Stephan Stadler.

\begin{pr}
Suppose $X$ is contactable metric with vanishing $(n+1)$-dimensional Hausdorff measure.
Assume $\Delta_1,\Delta_2\subset X$ be two embedded $n$-discs that have the same boundary.
...
where $\phi=\phi(x,y)$ is a positive smooth function on $\DD$ that takes huge values around the center and equals to 1 near the boundary of $\DD$.
...
the coefficients of $p$, up to a multiplicative constant, the integrals along $M$ of the quantities called \emph{Lipschitz--Killing curvatures} --- these are certain scalars that can be expressed in terms of curvature tensor at the given point.
...
\\ &=\ell_0 -\delta.
\end{align*}
Repeating the procedure few times we get we get curves $\gamma_2,\dots,\gamma_n$
...
Assume $f\:\Delta\to M$ is a map and $x\in M$ such that $f(\Delta)\subset B(x,1)_M$.
...
$\eps>0$.
Further, we can assume that all the vertices of $\tau$ can be colored into $m+2$ colors $(0,\dots, m+1)$
