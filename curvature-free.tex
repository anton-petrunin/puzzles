...
It is instructive to show that for $\mathbb{S}^2$ such thing is not possible. %??? make a problem from it.
...
$\T_u[\T M]$ for any $u\in \T_p M$ with the direct sum of vertical and horizontal subspaces $\T_p M\z\oplus \T_p M$.
...
Let $x\in \mathbb{S}^2$ be a point that minimize the distant $|x-x'|_g$.
Consider a minimizing geodesic $\gamma$ from $x$ to $x'$.
We can assume that 
\[|x-x'|_g=\length \gamma=2.\]

Let $\gamma'$ be the antipodal arc to $\gamma$.
Note that $\gamma'$ intersects $\gamma$ only at the common ends $x$ and $x'$.
Indeed, assume $p'=q$ for some $p,q\in\gamma$, then $|p-q|\ge 2$.
Since $\length \gamma=1$ we get that $p$ and $q$ are the ends of $\gamma$.

Therefore $\gamma$ together with $\gamma'$ forms a closed simple curve in $\mathbb{S}^2$
that divined it into two disks $D_1$ and $D_2$.

Let us divide $\gamma$ into two equal arcs $\gamma_1$ and $\gamma_2$; each has length $1$.
...
&\ge 2-1=1.
\end{align*}
That is, the minimal distance from $\gamma_1$ to $\gamma_1'$ is at least~$1$.
The same way we get that the minimal distance from $\gamma_2$ to $\gamma_2'$ is at least~$1$.
Applying Besicovitch inequality we get that 
\[\area(D_1,g)> 1\quad\text{and}\quad \area(D_2,g)> 1.\]
Therefore 
\[\area(\mathbb{S}^2,g)>2.\]
\qedsf

This inequality was proved by Marcel Berger \cite[see][]{berger}.
It seems that the optimal bound is admitted for a round sphere.
...
The same proof gives a ruf version of this inequality for any metric on the cube.
...
Following $\gamma$ for enuf time brings us to $p$;
...
Repeating the procedure enuf times we get we get curves $\gamma_2,\dots,\gamma_n$
