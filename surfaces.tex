...
in $\RR^3$ 
and $\Pi$ a plane that cuts from $\Sigma$ a disk $\Delta$.
...
Assume that $\Sigma$ passes through the center of the sphere.
...
One of the types can and the other cannot appear as self-intersections of a geodesic on an unbounded convex surface.
...
Note that the proof implies that a two-sided infinite geodesic can be found among geodesics that run through a given point in $\Sigma$.
...
of a geodesic on a convex surface \cite{liberman}.\footnote{It is a part of the thesis of Joseph Liberman, defended couple of months before his death in the WWII.}
...
The latter was proved inependently by Istv{\'a}n F{\'a}ry \cite{fary-knot} and  John Milnor \cite{milnor}.
