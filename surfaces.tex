...
Since $\Sigma$ is convex, the closest point projection shrinks the length of $\gamma$.
Therefore 
...
Recall that a closed curve 
 called \index{simple closed curve}\emph{simple} if it is given by an injective map from the circle to the space.
...
\emph{the concatenation of the line segment $[p_t\gamma(t)]$ and the arc $\gamma|_{[t,\ell]}$ forms a minimizing geodesic in the intrinsic metric induced on $W$}.
...
set $\alpha(t)=p_t$.
Note that $\dot\alpha(t)$ is perpendicular to $\Pi_t$
and it points to the half-space opposite from $\Sigma$.

It follows that for any $x\in\Sigma$ the the function  
\[t\mapsto |x - p_t|
...
It follows that the concatenation, say $\gamma_t$, of the line segment $[p_t\gamma(t)]$ and the arc $\gamma|_{[t,\ell]}$ forms a minimizing geodesic from $p_t$ to $q$ in the intrinsic metric of $W$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{Long geodesic.}
By cutting the surface $\partial B$ along a sufficiently long closed simple geodesic,
we get two discs with.
...
Since $\partial B$ has non-negative curvature in the sense of Alexandrov,
so is $D$.
Choose a pair of points $p$ and $q$ on $\partial D$ which maximize the distance.
Clearly,
\[|x-p|_D,|x-q|_D\le |p-q|_D\] 
for any other point $x\in D$.
By comparison, 
...
By comparison 
\[\area \tilde \triangle qxy\le \area \triangle qxy\le \area \partial B.\]
It follows that 
\[\begin{aligned}|x-y|_D&\le\frac{2\cdot \area[\tilde\triangle(xyq)]}{|q-x|_D}
\le 
\\
&\le 
\frac{100\cdot \area[\partial B]}{\length\partial D}.
\end{aligned}
\leqno({*}{*})\]

Cut from $D$ the loon $L_p$ with $p$ along a minimizing geodesic $[xy]$.
Note that the curvature of $L_p$ is $\alpha+\beta$, where $\alpha$ and $\beta$ the angles as on the diagram.
By comparison these $\alpha\ge \tilde\measuredangle(x\,^p_y)$ 
...
we can find choose a loop $L_p$ with diameter at most $\eps$,
such that curvature $L_p$ is at least $\pi-\eps$.

Using the same construction for $p$ and $q$ in the disc $D$,
and for the other disc,
we get four loons in $\partial B$ each of diameter at most $\eps$ and each with curvature at least $\pi-\eps$.
...
and the total curvature of $\partial B$ is $4\cdot\pi$.
...
generalizations were obtained in \cite{streltsov-alexandrov} by 
