...
Denote by $\kappa(\Omega)$ the integral of the Gauss curvaure along $\Omega$.

For any point $p\in\Sigma$ consider the outer unit normal vector $n(p)\in\mathbb{S}^2$.
Then 
\begin{align*}
...
is called \index{saddle surface}\emph{saddle} at the point $p$ if its principal curvatures at $p$ have opposite signs. 
...
Recall that an \index{asymptotic line}\emph{asymptotic line} on a smooth surface $\Sigma\subset \RR^3$
...
Note that the discs bounded by the loops  have to overlap,
...
Otherwise $\Sigma$ would fail to be saddle at the point in the disc surrounded by that cycle maximizing the distance to $\Pi$.
...
\begin{pr}
Show that a smooth bounded function $f\:\RR^2\to\RR$ can not havea  strictly saddle graph.
\end{pr}
...
of $\gamma$ to the $(x, y)$-plane is star shaped with respect to the origin.

Consider the function 
%<<<<<<< zamorabars-patch-1
%$$h(t)=(d_{\bar\gamma(t)}f)(\gamma(t)).$$ 
% I actually don't understand this, gamma is not in the domain of d_{\bar\gamma(t)}f and I dont get where the asymptotic line equation shows up.
%Direct calculations show that $h'(t)\ne 0$.
%=======
...
David Hoffman
and Robert Osserman 
proved it for two cases (1) for discs and (2) for arbitrary area minimizing surfaces, any dimension and codimension
