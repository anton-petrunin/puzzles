...
The \index{total curvature}\emph{total curvature} of $\gamma$ is defined as the integral of its curvature.
That is, if $t$ is the natural parameter, the total curvature of $\gamma$ is
\[\int\limits_a^b|\ddot\gamma(t)|\cdot dt,\]
the vector $\ddot\gamma(t)$ is called \index{curvature vector}\emph{curvature vector} and its magnitude $|\ddot\gamma(t)|$ is the \index{curvature}\emph{curvature} of $\gamma$ at time $t$.
The above definition has sense for $C^{1,1}$ smooth curves,
that is if $\ddot\gamma(t)$ is locally Lipschitz;
in this case the curvature $|\ddot\gamma(t)|$ defined almost everywhere.
...
Any geodesic in the problem forms a $C^{1,1}$ curve in $\RR^3$.
This is an easy in real analysis which can be taken as granted.
...
Let $\Sigma$ be a connected immersed surface in $\RR^3$ with strictly positive Gauss curvature and nonempty boundary $\partial\Sigma$.
...
and $\dot\alpha(t)$ points to the side of $\Pi_t$ opposite from $\Sigma$.
...
Cut $D$ along a minimizing geodesic $[xy]$ into two loons.
Denote by $L_p$ the loon with the point $p$ in it.
...
We can assume that $\gamma$ is defined on a closed interval $[0,a]$.
The key step is to show the following:
\medskip
\begin{itemize}
\item[$({*})$] The function $t\mapsto z$ is concave.
\end{itemize}
...
Note that concavity of $s\mapsto z$ is equivalent to the concavity of $t\mapsto z$.
Hence $({*})$ follows.

Since $f$ is smooth, 
the curve $\gamma(t)$ is $C^{1,1}$; 
that is, its first derivative $\gamma'(t)$ is Lipschitz.
...
It follows that 
\[\int\limits_0^a |z''(t)|\le 2\cdot\tfrac{\ell}{\sqrt{1+\ell^2}}.\]
...
Hence 
\[\int\limits_0^a |\gamma''(t)|\le 2\cdot\ell.\]
\qedsf

The statement holds for general $\ell$-Lischitz function,
not necessary smooth.
The given bound is optimal, check a both side infinite geodesic on the graph of  
...
\begin{lpic}[t(-5 mm),b(-0 mm),r(0 mm),l(0 mm)]{pics/bangert(1)}
\end{lpic}
\end{wrapfigure}

\parbf{Simple geodesic.} 
Look at two combinatoric types of self intersections shown on the diagram.
One of the curves can and the other can not appear as self intersections of geodesic on an unbounded convex surface.
...
In these coordinates $\Sigma$ is described as a graph $z=f(x,y)$ for a convex function $f$.
...
Part of $\Omega'$ touch the boundary of $K$ and part lies in the interior of $K$. 
Denote by $\gamma'$ the curve in $\Omega'$ dividing these two parts.
...
On the other hand $\gamma'$ is an asymptotic line.
If the direction of $\gamma(t)$ is not asymptotic
then it easily follows that $\gamma(t)$ lies in the interior of $K$.
...
I do not know any generic metric of that type.
