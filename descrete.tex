...
\begin{pr}
Assume that a rectangular parallelepiped with sides $a,b,c$ 
...
As you may guess from the diagram, the kissing number of the round disk in the plane is $6$.
...
Show that given $\eps>0$ there is a finite set of points $S$ that intersects every convex figure of measure at leaset $\eps$.
...
Let us give a short summary of the Dehn--Sommerville equations which can help to solve this problem.
...
Fix an ordering of the vertices $v_1,\dots, v_{f_0}$
...
The number of vertices of index $k$ will be denoted as $h_k$.
...
The identities $({*}{*})$ are called the Dehn--Sommerville equations.
...
Assume $\Pi'$ be another parallelepiped
with dimensions $a'$, $b'$ and~$c'$.
If $\Pi\subset \Pi'$,
...
can not exceed the average length of the projection of $\Pi'$.
...
Indeed, the set of equations of degree $d$ that correspond to singular curves have real codimesion 2.
...
(This projection reduces the problem to the one-dimensional case.)
...
where $3\cdot B_i$ denotes the ball with the same center as $B_i$ and with radius three times larger.
Hence the statement follows.
\qeds
...
The following statement is called the \emph{Besikovitch covering lemma};
...
such that center of any ball in $\{B_i\}_{i\in F}$ lies inside one of the balls from $\{B_i\}_{i\in G}$
...
Their applications overlap but aren't identical, \emph{Vitali covering theorem} works for nice measures in arbitrary metric spaces while \emph{Besikovitch covering theorem} work in nice metric spaces with arbitrary Borel measures.
...
\begin{pr}
Show the same for the surface of the higher dimensional cube.
\end{pr}
...
\begin{lpic}[t(-0 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/acute-triangles(1)}
\lbl[t]{2,-1;$x_i$}
\lbl[t]{20,-1;$x_j$}
\lbl[b]{13,9;$z$}
\lbl[t]{25,17;$z_i$}
...
Grigori Perelman noticed that the same proof works for a similar problem in Alexandrov spaces \cite[see][]{perelman-Erdos};
the later led to interesting connections with the crystallographic groups \cite[see][]{lebedeva}.


Surprisingly, the maximal number of points that make only acute triangles grows exponentially with $m$ as well.
The latter was shown by Paul Erd\H{o}s and Zolt\'an F\"uredi \cite[see][]{erdos-fueredi} using the \index{probabilistic method}\emph{probabilistic method}.
...
\cite[see][]{zakharov,grizzly,gerencser-harangi};
...
In particular its measure can not exceed $\tfrac34$.
...
This is a stripped version of a theorem proved by Boris Bukh and Gabriel Nivasch in \cite{bukh-nivasch}.
...
The above proof  
is the core of the proof of nonexistence of compact hyperbolic Coxeter's polyhedrons of large dimensions 
...
there is a regular right-angled  hyperbolic polyhedron with \index{120-cells}\emph{120-cells} --- a 4-dimensional uncle of the dodecahedra.

The following question is open.

\begin{pr}
Let $m$ be a large integer.
Can a cocompact group of isomtries on the $m$-dimensional Lobachevsky space be generated by finite order elements (for example by central symmetries)?
\end{pr}
