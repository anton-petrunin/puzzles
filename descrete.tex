...
Show that the cube has at least $2^{m-1}$ edges that connect the vertices of different colors. 
\end{pr}
...
Let us give a short summary of Dehn--Sommerville equations which can help to solve this problem.
...
Each $k$-face of $P$ contains a unique vertex that maximize $\ell$;
...
Indeed, the set equations of degree $d$ that correspond to singular curves have the real codimesion 2.
...
on each step we take the largest ball that does not intersect those which we choose already.
...
Let $B_1,\dots, B_n$ the copies of the ball $B$ that touch $B$ and have no common interior points.
...
It follows that there is a rotational symmetry of cube with order 3 that fix $v$ and sends $\xi$ to $\zeta$.
...
The proof above 
is the core of proof of nonexistence of compact hyperbolic Coxeter's polyhedrons of large dimensions 
...
there is a regular right-angled  hyperbolic \index{120-cells}\emph{120-cells} --- a 4-dimensional uncle of dodecahedron.
