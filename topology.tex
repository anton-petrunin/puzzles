...
This trick is used in the five-line proof of the Jordan separation theorem by Patrick Doyle \cite[see][]{doyle};
a proof of the separation theorem for the embeddings $\mathbb{S}^n\hookrightarrow\mathbb{S}^{n+1}$
can be built with the same idea \cite[see][]{cohen}. 

The problem ``Monotonic homotopy'' on page \pageref{mono-homotopy} looks similar.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Immersed discs}\label{Immersed discs}
...
\begin{pr} 
Construct two essentially different smooth immersions of the disc 
...
\begin{cl}{$({*})$} 
Assume $h$ is a positive Dehn twist along a closed curve $\gamma$.
...
each one is a closed annulus with two attached line segments.
...
A homeomorphism $K_1\times[0,1]\to K_2\times[0,1]$ can be constructed by twisting a part of one solid torus.
...
This problem is stripped from a proof of Robion Kirby \cite[see][]{kirby}.
...
he uses a discrete group of motions of the hyperbolic space in the conformal disc model.
...
such that
each $h_n$ fixes all the points outside an open round disc, say $D_n$.
...
\[\Omega\cap h(\Omega)\ne\emptyset\]
for any topological disc $\Omega\subset \SSS^2$ with area at least $\eps$
and 
