...
Let $\Sigma$ be a surface and $\gamma\:\RR/\ZZ\z\to\Sigma$ be non-contractible closed simple curve.
Let $U_\gamma$ be a neighborhood of $\gamma$ which admits a homeomorphism $\iota\:U_\gamma\to \RR/\ZZ\times (0,1)$.
...
such that
$$\iota\circ h\circ \iota^{-1}\:(x,y)\mapsto(x+y,y).$$
...
Consider the universal metric covering 
...
$y\prec z$ if $z$ lies on the left side from one simple curve from $x_0$ to $y$ in $\tilde\Sigma$.
...
Then $\tilde h(y)\preceq y$ for any $y\in\partial\tilde\Sigma\backslash\{x_0\}$
and $\tilde h(y_0)\prec y_0$ for some $y_0\in\partial\tilde\Sigma\backslash\{x_0\}$.

Moreover, $\tilde h(y_0)\prec y_0$ if any path from $x_0$ to $y_0$ has to cross $f^{-1}(\gamma)$.

\medskip

Note that the main property in the claim is a homotopy invariant 
and it survives under compositions of maps.
Therefore the problem is solved once we prove the claim.
