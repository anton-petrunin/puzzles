...
It remains to notice that any map $h\in \bigcap_F\mathfrak{C}_F$ solves the problem.
\qeds
...
Our argument also shows that there is no length-preserving map from an $n$-dimensional Riemannian manifold to $\RR^{n-1}$.
On the other hand, a theorem of Mikhael Gromov \cite[Section~2.4.11]{gromov-PDR} implies that \textit{any $n$-dimensional Riemannian manifold admits a length-preserving map into $\RR^n$}.
Another proof of this theorem is outlined in the previously mentioned lecture notes \cite{petrunin-yashinsky}.
...
Further, notice that the resulting spaces for two sequences $(a_n)$ and $(a'_n)$ are isometric only in the following two cases
\begin{itemize}
