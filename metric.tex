...
\begin{pr}
Show that for any compact length-metric space $X$ there is unique number $\ell=\ell(X)$ such that for any finite collection of points there is a point  average distance $\ell$ from the collection;
...
Then, by 1-dimesional Helly's theorem, there is a pair of point-arrays $\{x_1,\dots,x_n\}$ and $\{y_1,\dots,y_m\}$
such that for their average distance functions $f(z)=\tfrac1n\cdot\sum_i|x_i-z|$ and $h(z)=\tfrac1m\cdot\sum_j|y_j-z|$, we have 
$$f(p)>h(q)$$
for any two points $p,q$.

Note that 
$$\tfrac1m\cdot\sum_j f(y_j)=\tfrac1{m\cdot n}\cdot\sum_{i,j}|x_i-y_j|=\tfrac1n\cdot\sum_i h(x_i),$$
a contradiction.

To prove uniqueness, it is sufficient to approximate a constant function by average distance functions.
\qeds

This is a result of Oliver Gross \cite[see][]{gross}.
