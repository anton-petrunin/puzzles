...
In this case the metric on $X$ is called \index{length-metric}\emph{length-metric}.
\end{itemize}
...
Denote by $B(K,\RR)$ the space of real-valued bounded functions on $K$
equipped with sup-norm; 
that is, 
\[|f|=\sup\set{|f(x)|}{x\in K}.\]

Note that the map $K\to B(K,\RR)$, defied by $x\mapsto \dist_x$
is a distance preserving embedding.
The later follows since
by triangle inequality we have
\[|\dist_x(z)-\dist_y(z)|\le |x-y|_K\]
and the equality holds for $z=x$.

In other words we can (and will) consider $K$ as a subspace of $B(K,\RR)$ with the metric induced by sup-norm.

Denote by $W$ the linear convex hull of the image $K$ in $B(K,\RR)$;
that is $W$ is the intersection of all closed convex subsets containing $K$. 
Equip $W$ with the metric induced by the sup-norm.

Clearly $W$ is complete.
Since $K$ is compact we can choose a finite $\eps$-net $K_\eps$ in $K$.
Evidently convex hull $W_\eps$ of $K_\eps$ is compact and $W$ lies in the $\eps$-neighborhood of $W_\eps$.
...
A map $f\: X\to Y$ between metric spaces is called \index{non-contracting map}\emph{non-contracting} if
...
\begin{pr}{\easy}{Horo-compactification}\label{Horocompactification}
...
\begin{pr}{\easy}{Macroscopic dimension}\label{macroscopic dimension} 
...
the subspace $(U,d)$ does not admit a Lipschitz embedding into 3-dimensional Euclidean space.
\end{pr}
...
In addition we need to add the a condition
to ensures the following properties 
\begin{itemize}
\item The sub-Riemannian metric induce the original topology on the manifold; in particular, the distance cannot take infinite values.
\item Any curve in $M$ can be arbitrary well approximated by a horizontal curve with the same endpoints.
\end{itemize}
The most common condition of this type is so called  {}\emph{complete non-integrability};
it means that for any $x\in M$, 
one can choose a basis in its tangent space $\T_xM$
from the vectors of the following type
\[A(x),\  [A,B](x), [A,[B,C]](x), [A,[B,[C,D]]](x),\dots\] 
where $[{*},{*}]$ denotes the Lie bracket 
and all the vector fields $A,B,C,D, \dots$ are horizontal.
...
and moreover, the images of geodesics in this metric are arcs of great circles in $\mathbb{S}^2$.
\end{pr}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{A family of sets with no section\easy}
...
As an easy corollary one can get that any surjective short maps from compact metric space to it self is an isometry.
The following problem due to Aleksander Ca{\l}ka \cite{calka:loc-isom};
it is closely related but more involved. 
\begin{itemize}
 \item Show that any local isometry from a connected compact metric space to it self is a homeomorphism. 
\end{itemize}
...
denote by $\mathfrak{S}_F$ the set of all short maps $h\: F\to Y$ which agree with $f$ on $F\cap A$.
...
Without loss of generality we can assume that $A$ is a maximal set;
that is $f$ can not be extended to a bigger set in such a way that it satisfies the assumptions of the lemma. 
...
\end{itemize}
The claim is proved the same way as the Besikovitch inequality.

\medskip


Indeed, consider two functions of $X$ defined as
\begin{align*}
...
\begin{lpic}[t(-6 mm),b(1 mm),r(0 mm),l(0 mm)]{pics/pxy-tau(1)}
\lbl[t]{2,0;$p$}
\lbl[t]{35,0;$x$}
\lbl[t]{24,0;$m$}
\lbl[r]{.5,35;$y$}
...
Let $f\:\DD\to M$ be a disc contracting $\sigma$.
...
Note that for any 1-Lipschitz embedding $f\:(U,\rho)\to\RR^3$ the distance from $f(c_0)$ to $f(c_n)$ is less than $\ell$.
...
Also, it seems feasible that any metric on $\RR^3$ admits a local Lipschitz embedding into $\RR^4$.

Note that $\mathbb{S}^3$ with any metric admits a Lipschitz map to the round 3-sphere.
It is instructive to visualize this map for the metric as in the solution.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{Sub-Riemannian sphere.}
By the definition, if $d$ is a sub-Riemannian metric on $\mathbb{S}^m$,
...
Moreover, from the same construction it follows that
...
Then the strong condition is $\eps_n< \tfrac12\cdot r_n$, 
...
Therefore $\bar d(x,y)+\tfrac1{2^n}\ge d_n(x,y)$ for any $n$ and $x,y\in \mathbb{S}^m$;
hence $d'\ge d$ as required. 
\qeds
...
Therefore by Rademacher's theorem \cite[see][]{rademacher}, $f$ is differentiable almost everywhere.

Fix a unit vector $u$.
Given $x\in\RR^2$,
consider the curve $\alpha_x(t)\z=x+t\cdot u$ with $t\in [0,1]$.
Note that for almost all $x$ we have
...
In particular there is $x\in\RR^2$ such that the differential $d_xf$ is defined and has rank 2, a contradiction. \qeds 
...
Consider the map $\RR^3\to \RR^4$ defined as
$$f\:(x,y,z)\mapsto (x,y,x,z);$$
it has a left inverse 
$$h\:(x,y,x',z)\mapsto (\tfrac{x+x'}{2},y,z).$$
Fix a tangent vector $v$ of $\HH^3$.
Set $w=df(v)$ and so $v=dh(w)$.
Direct calculations show that $|v|\le |w|\le 2\cdot |v|$.
Hence the statement follows.\qeds
???

In the proof we used that horo-sphere in the hyperbolic space is isometric to the Euclidean plane.
...
\begin{lpic}[t(-0 mm),b(0 mm),r(0 mm),l(0 mm)]{pics/comb(1)}
\end{lpic}
\end{wrapfigure}

\medskip

First let us describe the the comb precisely.
...
If the equality holds for all $m$, cut the plane along the vertical half-plane from $(k,0)$.
...
two random sequences do the job with probability 1.\qeds