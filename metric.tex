...
Indeed,cby triangle inequality we have
\[|\dist_x(z)-\dist_y(z)|\le |x-y|_K\]
for any $z\in K$
and the equality holds for $z=x$.

In other words we can (and will) consider $K$ as a subspace of $B(K,\RR)$.
...
Clearly $W$ is a complete subspace of $B(K,\RR)$.
...
Evidently $f$ maps the line segment to a minimizing geodesic connecting $a$ to $b$ in $(\RR^m,\rho)$,
but in general there might be many minimizing geodesics connecting $a$ to $b$ in $(\RR^m,\rho)$.
...
The expected solution requires a corollary of the theorem  of Laurence Siebenmann \cite[see][]{siebenmann}.
It states that 
...
As an easy corollary one can get that any surjective non-expanding map maps from compact metric space to it self is an isometry.
The following problem due to Aleksander Ca{\l}ka \cite[see][]{calka:loc-isom};
it is closely related but more involved. 

\begin{pr}
Show that any local isometry from a connected compact metric space to it self is a homeomorphism. 
\end{pr}
...
denote by $\mathfrak{S}_F$ the set of all non-expanding map maps $h\: F\to Y$ which agree with $f$ on $F\cap A$.
...
Note that any metric on $\RR^3$ admits a proper locally Lipschitz map to $\RR^3$ of degree 1.
...
Note that for almost all $x$, the equality 
...
(A function $f\:X\to\RR$ is called affine if the composition $f\circ\gamma$ is affine for any geodesic $\gamma$ in $X$.)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{Hyperbolic space.}
The hyperbolic plane $\HH^2$
...
Note that 
\begin{align*}
|\phi(p)-\phi(q)|_{\HH^2},\quad |\psi(p)-\psi(q)|_{\HH^2}
&\le
|p-q|_{\HH^3}
\le
\\
...
two random sequences do the job with probability one.\qeds