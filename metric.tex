...
which is called \index{horocompactification}\emph{horocompactification}.
The complement 
...
\end{itemize}
The claim is proved the along the same lines as the Besikovitch inequality.
...
For a right choice of function $\eps(h_n)$,
...
The needed examples can be constructing from the upper half-plane
...
Note that the resulting spaces meet the conditions for the point $(\tfrac12,0)$.

Note yet that the resulting spaces for two sequences $(a_n)$ and $(a'_n)$ are isometric only in the following two cases 
(1) if $a_n=a_n'$ for all large $n$ 
and (2) if $a_n=1-a_n'$  for all large $n$.

It is easy to find two sequences which do not have these identities for all large $n$.\qeds