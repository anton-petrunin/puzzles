...
from a real interval into a space (for example, the Euclidean plane)
and 
...
of a standard introduction to differential geometry, [see Part I in \ncite{petrunin-zamora}, Chapter 1 in \ncite{toponogov-curves-and-surfaces}, or \S26--27 in \ncite{hilbert-cohn-vossen}].
...
The following problem shows that
if you drive in the plane while continuously turning the steering wheel in the same direction
(say, to the right), you cannot return to your starting point.
...
\begin{pr}
Show that for any $\eps>0$ there is $\delta>0$ such that any $\delta$-straight closed plane set $X$ is a curve that admits a locally $(1\pm\eps)$-bi-Hölder parametrization.
...
\noindent
\hskip-.9in
\arxiv{\includegraphics[scale=1]{mppics/pic-111}}{\includegraphics[scale=1]{mppics/pic-110}}

\vskip-.3in
Another example of a $\delta$-straight set is shown; it is constructed in a way similar to the Koch snowflake curve, where we used a very obtuse isosceles triangle instead of equilateral.
...
We need to explain the meaning of the word ``most'' in the formulation;
it uses \index{Hausdorff distance}\emph{Hausdorff distance} and \index{G-delta set}\emph{G-delta sets}.
...
\begin{pr}
Let $\gamma$ be a closed space curve with curvature at most 1 at every point.
...
Here is another problem I learned from Mikhael Gromov;
it can be solved using an idea of Berk Ceylan \cite{ceylan}.

\begin{pr}
Suppose $\gamma$ is a smooth closed plane curve with zero turning number and curvature less than $1$ at every point.
Let $F$ be the complement of the unbounded component of $\RR^2 \setminus \gamma$.
Show that $F$ contains two disjoint unit discs.
\end{pr}

}

The 3-dimensional analog of this statement does not hold in general.
Namely, there exists a smooth embedded sphere in $\RR^3$ with principal curvatures lying between $-1$ and $1$, yet it does not enclose an open ball of radius~$1$.
An example can be obtained by thickening a nontrivial contractible 2-complex in $\RR^3$
[Bing's house, constructed in \ncite{bing} will do the job].
Such examples are discussed by Abram Fet and Vladimir Lagunov \cite{lagunov-2,lagunov-fet-1,lagunov-fet-2}.
They were later generalized to Riemannian manifolds with boundary by Stephanie Alexander and Richard Bishop \cite{alexander-bishop}.

The same reason stands behind the difference between dimensions 2 and 3 in the following statements.
For any Riemannian metric $g$ on the 2-sphere $\mathbb{S}^2$
and any point $p \in (\mathbb{S}^2, g)$, there exists a minimizing geodesic $[pq]$ with conjugate endpoints.
In contrast, this is not true for $(\mathbb{S}^3, g)$.
Moreover, there exists a metric $g$ on $\mathbb{S}^3$
with sectional curvature bounded above by an arbitrarily small $\eps > 0$, and with $\diam(\mathbb{S}^3, g) \le 1$.
In particular, $(\mathbb{S}^3, g)$ contains no minimizing geodesics with conjugate endpoints.
An example was originally constructed by Mikhael Gromov \cite{gromov-almost-flat};
a simplified version was given by Peter Buser and Detlef Gromoll \cite{buser-gromoll}.

Let us now formulate several weaker 3-dimensional analogs of the problem.

\begin{pr}
Show that a smooth closed surface $\Sigma$ in $\RR^3$ with all principal curvatures between $-1$ and $1$ encloses a ball of radius~$1$, provided it satisfies one of the following conditions:
\begin{enumerate}[(a)]
 \item $\Sigma$ is a surface of revolution,
 \item $\Sigma$ is convex, or
 \item $\Sigma$ lies inside a ball of radius 2.
\end{enumerate}
\end{pr}

All three cases are discussed in \cite{petrunin-zamora}; the last problem is inspired by the work of Hongda Qiu.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\parbf{Wire in a tin.}
To solve this problem,
you should imagine that you travel on a train along the curve $\alpha$
...
\parit{A solution with Crofton's formula.}
...
The first identity above is called \index{Crofton's formula}\emph{Crofton's formula}.
...
A closely related {}\emph{equal tangents problem} is discussed by the same author \cite{tabachnikov-tan}.
...
This problem was posed by Zarathustra Brady.
The solution presented here is based on an idea of David Eppstein \cite{zeb}.
The statement also follows from a much earlier result of Mikhail Pronin \cite{pronin}.
...
These disks are called \index{lakes of Wada}\emph{lakes of Wada}; it is described by Kuniz\^{o} Yo\-ne\-ya\-ma \cite{yoneyama}.
...
By shifting the origin, we can assume that $\int_{x \in D} f(x) \cdot x = 0$.
In this case,
\begin{align*}
0
