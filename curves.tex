...
Note that $\alpha^{*}$ passes through two antipodal points of the sphere, the
...
Construct recursively a sequence of simple closed curves $\gamma_n\:[0,1]\z\to\RR^2$ such that $\gamma_{n+1}$ runs outside of the disc bounded by $\gamma_n$
and 
...
Denote by $D$ the union of all discs bounded by the curves $\gamma_n$.
...
\begin{pr}
 Construct three open discs in $\RR^2$ having the same boundary.
\end{pr}
...
In fact, this is a very common situation --- we do not usually see the typical objects and these objects often have surprising properties.
