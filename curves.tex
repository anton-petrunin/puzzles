...
\begin{pr}
Show that for any $\eps>0$ there is $\delta>0$ such that any $\delta$-straight closed plane set $X$ admits a locally $(1\pm\eps)$-bi-H\"older parametrization.
That is, for any $x\in X$ there is a curve $\alpha\:[0,1]\to \RR^2$ that covers a neighborhood of $x$ in $X$ and the inequality
...
\hskip-30mm\includegraphics{mppics/pic-2}

Another example of $\delta$-straight set is shown; it is constructed in a way similar to Koch snowflake curve, where instead of equilateral triangle we use a very obtuse isosceles triangle.
...
Note that any for $\eps$-straight set $X$ and any $r>0$ there is a $r$-reachable set $L$ 
that is $\eps\cdot r$-close to $X$ in the sense of Hausdorff.
Construct a sequence of such approximations $L_n$ for $r_n=\tfrac1{2^n}$.
Note that $L_n$ lies in a $2\cdot\eps\cdot r_n$-neighborhood of $L_{n+1}$.
It follows that the closest point porojection $f_n\:L_n\to L_{n+1}$ is uniquely defined.
...
\[
\begin{aligned}|x_{n+1}-x_n|&<2\cdot\eps\cdot r_n,
&
|x_{n+1}-y_{n+1}|&\gtrless(1\mp\eps)\cdot |x_{n}-y_{n}|.
\end{aligned}
\eqno({*})
\]
In particular, for any $x_1\in L_1$, the sequence $(x_n)$ converges in it-self; denote its limit by $x_\infty$. consider the map $F\:x_1\mapsto x_\infty$.
Evidently $F\:L_1\to X$ is onto.

Note that inequalities $({*})$ imply that 
\[
\begin{aligned}
|x_{n+1}-y_{n+1}|\le \min\{(1+\eps)\cdot |x_{n}-y_{n}|, |x_{n}-y_{n}|-4\cdot \eps\cdot r_n\},
\\
|x_{n+1}-y_{n+1}|\ge \max\{(1-\eps)\cdot |x_{n}-y_{n}|, |x_{n}-y_{n}|+4\cdot \eps\cdot r_n\}
\end{aligned}
\eqno({*}{*})
\]
...
A very similar construction is used in the proof of Nash--Kuiper theorem about length preserving $C^1$-embeddings \cite{nash,kuiper}.
