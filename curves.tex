...
In fact there are connected set in the plane (for examle pseudoarc) that contain no curves.
...
Another example of $\delta$-straight set is shown; it is constructed in a way similar to Koch snowflake curve, where we used a very obtuse isosceles triangle instead of equilateral.
...
Choose the a chord that divides the area of bigger oval in the minimal (or maximal) ratio.
...
A solution of the following problem is based on the same idea.
...
The the total length of the constructed curves outside of $X$ can not exceed 
\[\eps_1+\eps_2+\dots;\]
hence the result.
...
By the regularity of length, we may cover $\Delta\cap X$ by a finite collection of arcs with total length arbitrary close to the length of $\Delta\cap X$.
...
two vertexes $\alpha_i$ and $\alpha_j$ are connected by an edge if there is a connected set $\Theta\subset  X\cap Q_k$ 
...
Is it always possible to connected $x$ and $y$ by a path $\alpha$ such that the complement $\alpha\backslash X$ has arbitrary small length?
\end{pr}
