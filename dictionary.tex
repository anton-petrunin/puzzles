...
\item{\bf Busemann function.}\refstepcounter{thm}\label{Busemann function} Let $X$ be a metric spaces
...
\item{\bf Doubling}\refstepcounter{thm}\label{Doubling} of a manifold $M$ with boundary $\partial M$ is is two copies of $M_1, M_2$ identified along corresponding points on their boundaries $\partial M_1, \partial M_2$.
...
\item{\bf Length preserving map.}\refstepcounter{thm}\label{Length preserving map} A map $f\:X\to Y$ of \hyperref[Length space]{\emph{length spaces}} $X$ and $Y$ is a length preserving map if for any path $\alpha\:[0,1]\to X$, we have 
$$\length(\alpha)=\length(f\circ\alpha).$$

\item{\bf Minimal surface.}\refstepcounter{thm}\label{Minimal surface} Let $\Sigma$ be a $k$-dimensional smooth surface in
...
\item{\bf Polyhedral space}\refstepcounter{thm}\label{Polyhedral space} --- a simplicial complex with a metric such that each simplex is isometric to a simplex in a Euclidean space.
...
\item{\bf Short map}\refstepcounter{thm}\label{Short map} --- the same as 1-Lipschitz of distance nonexpanding map.
