...
Given a tangent vector $X\in \T W=\T M\times \T N$ denote by 
$X_M\in \T M$ and $X_N\in \T N$ its projections.
Let us equip $W$ with the Riemannin metric defined as
\[s(X,Y)=g(X_M,Y_M)+f^2\cdot h(X_N,Y_N).\]
The obtained Riemannin manifold $(W,s)$ is called warped product of $M$ and $N$ with respect to $f\:M\to \RR$;
the later can be written as  $(W,g)=(N,h)\times_f(M,g)$.
