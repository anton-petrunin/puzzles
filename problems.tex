...
\begin{pr}{\hard}{Poincar\'e's last theorem}\label{Poincare's last theorem}
...
\begin{pr}{}{Geodesic hypersurface}\label{Geodesic hypersurface}
...
Show that the space of nonnegatively curved left invariant metrics on a given compact Lie group is contractible.
\end{pr}
...
and any $1$-Lipschitz function $f\:M\to\RR$ the following holds.

For a random unit-speed geodesic $\gamma$ in $M$ 
the probability that 
\[|f(\gamma(0))-f(\gamma(1)|>\eps\]
is at most $\eps$.
Here \emph{random} means that $\gamma'(0)$ takes the random value in the unit tangent bundle of $M$.
\end{pr}
...
Assume that $f\:(\RR^m,d)\to(\RR^m,d)$ is an isometry which fixes two distinct point.
...
Let $\rho$ be a length-metric on $\RR^m$, which is bi-Lipschitz equivalent to the canonical metric.
...
Consider the tangent bumbde $\mathrm{T} \mathbb{S}^2$ 
...
{%???CHANGE PIC
\begin{wrapfigure}[4]{o}{25mm}
%\begin{center}
\begin{lpic}[t(-8mm),b(-4mm),r(0mm),l(0mm)]{pics/Sierpinski-Trigon(.125)}
%\lbl[b]{24,41;x}
\end{lpic}
%\end{center}
\end{wrapfigure}
...
denote by $P$ the quotient $\RR^m/\Gamma$ equipped with induced \hyperref[Polyhedral space]{\emph{polyhedral metric}}.
...
Denote by $\square^m$ a cube in $\RR^m$.
