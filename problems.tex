...
Let $f\:\RR^n\to\RR^n$ be a homeomorphism with displacement at most $\eps$;
that is $|x-f(x)|\le\eps$ for any $x\in\RR^n$.

Construct a homomorphism $h\:\RR^n\to\RR^n$ 
with displacement at most $\eps$ such that
 $h(x)=f(x)$ if $|x|\le 1$ 
and  $h(x)=x$ if $|x|\ge 2$.
\end{pr}
