...
A \index{polyhedral space}\emph{polyhedral space} is complete length-metric space that admits a locally finite triangulation 
...
A triangulation of a convex polygon is called coherent if there is a convex function that is linear on each triangle and changes the gradient on every edge of the triangulation.
...
The polyhedron $P$ above is an example of so called \index{cyclic polytope}\emph{cyclic polytopes}.
...
Moreover, the surface that minimize the area among all surfaces with fixed  triangulation might be not saddle;
...
The given example is closely related to the so called \index{Sch\"onhardt polyhedron}\emph{Sch\"onhardt polyhedron}, an example of non-convex polyhedron which does not admit a triangulation \cite[see][]{schoenhardt}.
...
Consider a closed geodesic $\gamma_1$ that minimize the length of all circles which are not null-homotopic.
Let $\gamma_2$ be an other closed geodesic that minimize the length and is not homotopic to any power of $\gamma_1$.
...
In fact no geodesic can pass thru singular point with total angle smaller than $2\cdot\pi$.

Assume $\gamma_i$ passes thru $z_7$.
Then by $({*})$, one of two angles cut by $\gamma_i$ at $z_7$ equals to $\pi$.
...
Note that the neighborhood of the first boundary component is parallelogram --- it has equal opposite sides and its angles add up to $2\cdot \pi$.
...
that have the total angles $\tfrac53\cdot\pi$ and $\tfrac73\cdot\pi$.
...
This example shows that, the observation $({*})$ is essential in the proof.
...
This problem was stated and solved by Gabirele Mondello and Dmitri Panov in \cite{mondello-panov}.
The given solution relies on an other holonomy group.
It gives an element of the double cover of $\SO(3)$ (which is isometric to $\mathbb{S}^3$) to any loop in $\Sigma$ that avoids singularities.
...
By Gauss--Bonnet formula, a simple closed geodesic cuts the surface into two discs with total curvature $2\cdot\pi$ each.
