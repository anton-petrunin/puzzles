...
It follows from very general result of Burago and Zalgaller \cite[see][]{burago-zalgaller:pl}.
...
Assume it is coherent and $f$ is the corresponding piecewise linear convex function.
Without loss of generality we can assume that $f$ vanish on the boundary of big triangle.
Arrive to a contradiction by showing 
\[f(x)>f(y)>f(z)>f(x).\]
\qedsf
...
The given example is closely related to so called \index{Sch\"onhardt polyhedron}\emph{Sch\"onhardt polyhedron}, an example of non-convex polyhedron which does not admit a triangulation \cite[see][]{schoenhardt}.
...
If a the surface of $P$ contains arbitrary long closed simple geodesic, 
then $P$ is \emph{isosceles tetrahedron};
that is, a tetrahedron with equal opposite edges.
This statement was was proved by Vladimir Protasov in \cite{protasov}
and generalized for arbitrary convex bodies in \cite{akopyan-petrunin} by Arseniy Akopyan and Anton Petrunin.
