...
in other words, the polyhedral space can be chosen to be quotient of $\mathbb{R}^3$ by a crystallographic action.
...
Consider the action generated by order-3 rotations around two diagonals of the cubes shown on the diagram.
Note that this action is crystallographic;
in fact it preserves the cubical grid.
Therefore the quotient by this action, say $P$, is a compact polyhedral space.

Note that isotropy group of any point is either trivial or has order 3.
Therefore any point of $P$ admits a neighborhood that is isometric to an open set either in $\RR^3$ or $\RR^3/\ZZ_3$.
Both of these spaces are topological manifolds;
therefore, $P$ is a 3-dimensional manifold. 

Since the action is generated by rotations, $P$ is simply connected.
By the Poincaré conjecture, $P$ is a topological sphere.

The singular locus of $P$ is the image if the axes of all the order-3 rotations.
Note that this action is transitive on the set of all these axes.
It follows that the singular locus $P_s$ of $P$ is connected; that is, $P_s$ is a circle.

\medskip

This action is called P$2_13$; it has number 198 in the classification.
Among 219 crystallographic actions, this is the only one that has the required property.
The latter follows from the classification given by William Dunbar \cite{dunbar}.

Note that a 3-fold covering of $P$ branching in $P_s$ is a flat manifold. 
In particular this covering is \emph{not} simply connected.
Therefore $P_s$ is not a trivial knot.
In fact this is the figure-eight knot.
...
The are a few crystallographic actions with singular locus formed by links. 
...
Other examples, nonorbifold, shperical, hyperbolic are discussed by Michel Boileau and Joan Porti \cite[Chapter 9]{boileau-porti};
more examples are given by Alexander Mednykh \cite{mednykh}.

I do not see a straightforward generalization of this problem to higher dimensions.
But polyhedral Kähler manifolds seem to be one of the possibilities.
Тhese are polyhedral manifolds with complex structure on each simplex of maximal dimension that agrees on each simplex of codimension 1 (plus a minor condition);
they were defined and studied by Dmitri Panov \cite{panov-Kaeler}.
In particular he constructed plenty examples of polyhedral Kähler metrics on 
$\CP^2$;
...
Let me describe a link between polyhedral Kähler metrics and the original problem;
this is why I was interested in the original problem at the first place.
The spherical polyhedral 3-spaces with singular locus formed by knot or link appear as spaces of directions in 4-dimensional Kähler manifolds.
In addition such spaces have an isometric $\mathbb{S}^1$-action induced by the complex structure on the manifold.
