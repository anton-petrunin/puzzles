...
The surface of $P$ is homeomorphic to $\mathbb{S}^3$.
...
\parit{Sketch of a complex-analytic proof.}
Assume that such a triangulation exists.
The equilateral triangles endow the torus with a complex structure, so the torus can be identified with a quotient of $\CC$ by a lattice.

Let us map each triangle to a fixed equilateral triangle in $\CC$ by an orientation-preserving isometry.
The resulting map $f$ is defined separately on each triangle, and the derivatives on adjacent triangles differ by multiplication by a sixth root of unity.
It follows that $w=(df/dz)^6$ is a meromorphic function;
moreover, $w$ has one simple zero and one simple pole, located at the two exceptional vertices.

Hence $w$ defines a degree-one holomorphic map from the torus to the sphere, which is impossible.
\qeds

The problem was originally posed and solved by {\fontencoding{T1}\selectfont Stanislav Jendro\v{l}}
and Ernest Jucovi\v{c} \cite{jendrol-jucovich};
their proof is combinatorial.
Our main solution follows a lecture by Rostislav Matveev \cite{matveyev}.
The complex-analytic proof was found by
Ivan Izmestiev, 
Robert Kusner, 
Günter Rote, 
Boris Springborn, 
...
We need to find unit vectors $v$ and $w$ such that $\langle Lx, w \rangle = \langle x, v \rangle$ for all $x$;
in other words, $v = L^*w$ and $|L^*w| = |w| = 1$, where $L^*$ denotes the adjoint of $L$.
Once this is done, it remains to rotate the simplices so that both $v$ and $w$ point in the vertical direction.

Note and use that $L^*$ is also volume-preserving.
\qeds

The two-dimensional version of this problem (with a more elementary but rather awkward proof) appears in the collection of Paul Halmos \cite[5J]{halmos}.

With a bit more work, one can position the simplices so that, for any $k$, their intersections with any coordinate $k$-plane spanned by the first $k$ elements of the standard basis vectors in $\RR^n$ have the same $k$-volume.
In this case, $L$ is represented by an upper triangular matrix with units on the diagonal.
Of course, the same holds for any pair of affine-equivalent bodies of equal volume.
