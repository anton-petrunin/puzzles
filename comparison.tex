...
The space of geodesics starting at $z$ is can be identified with the sphere $\mathbb{S}^m$.
...
We will use the following rough volume comparison inequality; it can be proved along the same lines as the Bishop--Gromov inequality.

Let $M$ be a complete $n$-dimensional Riemannian manifold with nonnegative Ricci curvature.
Given real numbers $0<\alpha<\beta<\gamma$, there is a constant $C=C(n,\alpha,\beta,\gamma)$ that satisfies the following condition:
Suppose any point of a region $R\subset M$ lies on distance between $\beta$ and $\gamma$ to a fixed point $q$.
Let $S$ be a hypersurface in $M$ that lies on the distance at least $\alpha$ from $q$ and intersects every geodesic from $q$ to $R$.
Then  \[\area S\ge C\cdot\vol R.\]
...
By the volume comparison, %???spell
we get 
