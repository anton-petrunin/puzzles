...
The expected solution use the following relative version of Bochner formula.
Assume $M$ a Riemannian manifold with boundary $\partial M$.
...
In particular the universal metric cover $\tilde W$ of $W$ is a $\CAT[0]$ space.
...
Assume $f$ is smooth.
If discs are meeting at angle at most $\eps<\tfrac\pi2$,
...
\kappa\cdot|\nabla f|^2,\]
where $K$ and $\kappa$ denotes the curvature of $D$ and geodesic curvature of $\partial D$ correspondingly.
...
Finally if $f$ is not smooth, one can apply to it the smoothing theorem of Greene and Wu \cite[see Theorem~2 in][]{greene-wu} and the remaining part of proof still works.
\qedsf

This estimate was used by Nina Lebedeva and the author \cite[see][]{lebedeva-petrunin-curvature}.
There are of cause other places in comparison geometry, where Bochner's formula shows up.
Various vanishing theorems and estimates for eigenvalues of Laplacian are the most famous examples \cite[see II \S 8 in][]{lawson-michelsohn}.
