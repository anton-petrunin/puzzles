...
The \index{Busemann function}\emph{Busemann function} $\bus_\gamma\:M\to\RR$ is defined by
...
\begin{pr} Let $M$ be a complete simply connected manifold with negatively pin\-ched sectional curvature
...
moreover, its boundary $\partial N'_s$ can be made arbitrary close in the $C^\infty$-metric to $\partial N$.
...
In particular $\Sigma_r$ is embedded embedded.
...
in particular the mean curveture vector of $\Sigma$ at $y$ can not vanish, a contradiction.
...
Let $\bus\:M\to\RR$ be the Busemann function such that $\Sigma=\bus^{-1}(\{0\})$.
...
defined below with a left-invariant metric.
...
These two statements proved by Theodore Frankel \cite[see][]{frankel}.
...
Assume $p\in X$ corresponds to a fixed point $\bar p\in M$ of $\mathbb{S}^1$-action.
...
this is straightforward but requires work.
...
It remains to observe that $(N,g_n)$ converges to $(M,g)/G$ as $n\to \infty$.\qeds

This construction is called sometimes \index{Cheeger's trick}\emph{Cheeger's trick}.
The earliest use of it I found it \cite{GKM}; it was used there to show that Berger's spheres have positive curvature.
...
Given $x\in M$ denote by $f(x)$ a point such that 
\[|x-f(x)|_g+|p-f(x)|_g\]
...
If $p<2$, then straightforward calculation show that 
$$f(x)=\sum\phi(|q-x|_{\ell_p})$$
where the sum is taken for all $q=\pm\eps\cdot e_i$,
has strict local minimum at $0$ and $\eps>0$ is small.
If $p>2$, one has to take the same sum for  $p=\sim_i\pm\eps\cdot e_i$ for all choices of signs.
...
Note that the angles of the triangle $[abc]$ coincide with its model angles.
The latter follows since the direction to the coordinate planes are parallel along the coordinate lines;
therefore the corresponding triangle in the Euclidean coordinate space has the same angles.

Both curvature conditions imply that there is a solid flat geodesic triangle in $(\RR^3,g)$ with vertex at $a$, $b$ and $c$.
...
This problem is based on a lemma discovered by Sergei Buyalo in \cite[Lemma 5.8 in][see also]{buyalo,panov-petrunin}
appears in the paper of Dmitri Panov and me \cite[see][]{panov-petrunin}; 
it 
...
According to the main result of Alexander Bishop and Berg in \cite{ABB}, $W\z=\RR^4\backslash (\Int K)$ has non-positive curvature in the sense of Alexandrov.
